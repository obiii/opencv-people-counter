...
parser.add_argument("--webcam", help="use webcam",action="store_true")
parser.add_argument("--save", help="file name to save",action="store")
...
# Creating hog configurations here
...
# video write variable initilization
fps = 20
videoWriter = None

#log file initilaization
if args.generateLog==True:
    logger = logging.getLogger('PeopleCountApp')
    logFile = "logfile"+str(time.strftime("%c"))+".log"
                    ...
                    if prevInStatus != currInStatus and people.isInChangeFrameCount >= toleranceCountIOStatus:
                        ...
                        if currInStatus:
                            ...
                            if not allowPassage:
                                peopleViolationIn += 1
                                if args.generateLog==True:
                                    logger.warning('Person Voilation Detected')
                        else:
                            peopleOut += 1
                            if args.generateLog==True:
                                logger.info('Person out Detected')
                            if not allowPassage:
